...
- Package manager is pnpm with `packageManager` pinned in `package.json`; Node is pinned via `.nvmrc` and `engines` (use `scripts/use-nvm.sh` when the shell does not load nvm); lint/format uses **Ultracite** (Oxlint + Oxfmt; see `oxlint.config.ts`, `.oxfmtrc.jsonc`).
