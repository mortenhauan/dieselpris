...
- When adding a news section, keep the newest article first, give each article its own URL, and reuse the shared site header so editorial pages feel like part of the same product.
- News articles must read like journalism, not meta-commentary about sources. Write the story for the reader (truck driver, fisher, fleet owner) who will never click through to Stortinget or regjeringen.no. Tell them what happened, what changes, when, and how much. Each article should be a self-contained static route with its own page.tsx, not a shared data object rendered through a generic template.
