...
export interface NorgesBankUsdNokSeries {
...
export const usdNokOnOrBefore = function usdNokOnOrBefore(
  series: NorgesBankUsdNokSeries | null,
  ymd: string,
  fallback: number
): number {
  if (!series || series.sortedDatesAsc.length === 0) {
    return fallback;
  }
  ...
  const [first] = sortedDatesAsc;
  return byDate.get(first) ?? fallback;
};

export const fetchNorgesBankUsdNokSeries =
  async function fetchNorgesBankUsdNokSeries(
    ...
    const [ds0] = parsed.data.data.dataSets;
    if (!ds0) {
      return null;
    }
    ...
    const [firstSeries] = seriesEntries;
    const { observations } = firstSeries;
    const timeValues =
      parsed.data.data.structure.dimensions.observation[0]?.values;
    ...
    for (const [idxStr, valArr] of Object.entries(observations)) {
      ...
      if (!Number.isFinite(idx) || idx < 0 || idx >= timeValues.length) {
        continue;
      }
    ...
    const latestDate = sortedDatesAsc.at(-1);
    if (latestDate === undefined) {
      return null;
    }
    const latestRate = byDate.get(latestDate);
    if (latestRate === undefined || !Number.isFinite(latestRate)) {
      return null;
    }

    return { byDate, latestDate, latestRate, sortedDatesAsc };
  };
